...
- Storage
    1. The values written to storage are persistent between calls.
    2. Storage is expensive to use. Using command, SSTORE will cost upto 20,000 gas.

- Memory
    1. The valuse written to memory exists only during a method execution and when the method execution finishes, this value will disappear.
    2. Memory is cheap to use. Using command, MSTORE will only cost 3 gas.
...
- implement the new method vote(string), which will allows user to vote not by providing 
  an option position but by providing an option position.
